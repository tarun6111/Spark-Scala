...
object DeviceCountPowerState {
  ...
  def main(args : Array[String]) {
    ...
    val spark = SparkSession
                .builder
                .appName("DeviceCountPowerState")
                .master("local")
                .getOrCreate()
    
    val data = spark.read.textFile("/Users/tarun.madimi/Desktop/Adhoc/Scala/DataSet/Set_Top_Box_Data.txt").rdd
    
    val result = data.filter(line => line.split("\\^")(2).toInt == 101).map( line => {
       val rec = XML.loadString(line.split("\\^")(4))
       val rec1 = rec \\ "nv"

       var powerState = ""
       for (i <- 0 to rec1.length-1 if rec1(i).toString.contains("PowerState")) powerState = (rec1(i) \\ "@v").toString;
